...
use crate::color::{book_image, build_glyph_palette, room_accent};
...
fn accent_for(z: i64, n: i64, universe_seed: u64) -> (f64, f64, f64) {
    let a = room_accent(z, n, universe_seed);
    (a[0], a[1], a[2])
}
...
fn book_grid_matches_book_image() {
    ...
    set_universe(0);
    let img = book_image(0, 0, 0, DEFAULT_ALPHABET);
...
fn round_trip_book_image_exact_accent_near_perfect_fit() {
    ...
    let (z, n, book) = (1_i64, 2_i64, 3_u32);
    ...
    assert!(
        hit.location.z != z || hit.location.n != n || hit.location.book_index != book,
        "locate dest should differ from export origin"
    );
}
...
fn babel_cross_universe_same_fit_different_coords() {
    let _g = lock_universe();
    let alphabet_id = DEFAULT_ALPHABET;
    let (z, n, book) = (5_i64, -3_i64, 11_u32);
    set_universe(0);
    let img = book_image(z, n, book, alphabet_id);
    let pixels = img.pixels();
    let ab = alphabet(alphabet_id);
...
fn wrong_accent_fit_is_noticeably_worse() {
    ...
    set_universe(0);
    let alphabet_id = DEFAULT_ALPHABET;
    let img = book_image(1, 2, 3, alphabet_id);
    let pixels = img.pixels();
    let ab = alphabet(alphabet_id);
    let space_idx = alphabet_space_idx(ab);
    let (hue, chroma, light) = accent_for(1, 2, 0);
...
fn oklab_nearest_prefers_matching_accent_palette() {
    ...
    set_universe(0);
    let alphabet_id = DEFAULT_ALPHABET;
    let img = book_image(9, 2, 0, alphabet_id);
    let pixels = img.pixels();
    let ab = alphabet(alphabet_id);
    let space_idx = alphabet_space_idx(ab);
    let (hue, chroma, light) = accent_for(9, 2, 0);
    let good = build_glyph_palette(ab, hue, chroma, light);
...
fn perceptual_fit_prefers_matching_accent_over_wrong_hue() {
    ...
    set_universe(0);
    let alphabet_id = DEFAULT_ALPHABET;
    let img = book_image(4, 5, 6, alphabet_id);
    let pixels = img.pixels();
    let ab = alphabet(alphabet_id);
    let space_idx = alphabet_space_idx(ab);
    let (hue, chroma, light) = accent_for(4, 5, 0);
    let good = build_glyph_palette(ab, hue, chroma, light);
    let bad = build_glyph_palette(ab, (hue + 180.0) % 360.0, chroma, light);
    let (_, mosaic_good) = project_indices(&pixels, &good, space_idx, 0.0, false);
    let (_, mosaic_bad) = project_indices(&pixels, &bad, space_idx, 0.0, false);
...
fn coarse_downsample_preserves_ranking_signal() {
    ...
    set_universe(0);
    let alphabet_id = DEFAULT_ALPHABET;
    let img = book_image(8, 1, 2, alphabet_id);
    let pixels = img.pixels();
    let ab = alphabet(alphabet_id);
    let space_idx = alphabet_space_idx(ab);
    let (hue, chroma, light) = accent_for(8, 1, 0);
